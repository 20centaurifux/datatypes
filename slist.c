 ...
 */
SList *
slist_new(CompareFunc compare, FreeFunc free, Allocator *allocator)
{
	SList *list;
	...
	slist_init(list, compare, free, allocator);

	return list;
}

void
slist_init(SList *list, CompareFunc compare, FreeFunc free, Allocator *allocator)
{
	assert(list != NULL);
	assert(compare != NULL);
